...
class Tile {
  Tile(this.x, this.y, this.z, {required this.image, required this.future});
  int x;
  int y;
...
class _FlutterEarthState extends State<FlutterEarth>
  ...
  late double _lastGestureScale;
  late double _lastGestureRatation;
  ...
  late Animation<double>? panAnimation;
  late Animation<double>? riseAnimation;
  ...
  late List<HashMap<int, Tile>> tiles;
  ...
  Future<Tile> loadTileImage(Tile tile) async {
    ...
    widget.onTileStart(
        tile); // onTileStart är required, så det kommer inte att vara null
    ...
    widget.onTileEnd(
        tile); // onTileEnd är required, så det kommer inte att vara null
    if (mounted) setState(() {});
  ...
  Tile getTile(int x, int y, int z) {
    ...
    if (tile == null) {
      tile = Tile(x, y, z, image: null, future: null);
      tiles[z][key] = tile;
    }
    ...
    if (tile.status != TileStatus.ready) {
      for (int i = z; i >= 0; i--) {
        ...
        if (tile1 != null && tile1.status == TileStatus.ready)
          return tile1; // check if tile1 is not null before returning
      }
      // Handle the case where no suitable Tile is found.
      // You might need to return a default Tile or handle this case differently.
      return tile; // make sure to return a non-null Tile
    }
    return tile;
  }
  ...
  List<Offset> clipTiles(Rect clipRect, double radius) {
    ...
    final lastKeys = List<int>.filled(clipRect.width ~/ 10 + 1,
        0); // 0 är det värde som listan kommer att fyllas med initialt
  ...
  void initMeshTexture(Mesh mesh) {
    ...
    if (tile.status == TileStatus.ready) {
      ...
      if (tile.z != zoomLevel) {
        ...
        final num scale = math.pow(2, tile.z - zoomLevel);
  ...
  Mesh initMeshFaces(Mesh mesh, int subdivisionsX, int subdivisionsY) {
    ...
    final List<Polygon> faces = List<Polygon>.filled(
        faceCount, Polygon(0, 0, 0, 0.0),
        growable: true); // Initialize with default Polygon
    ...
    // Konverterar Polygon-listan till Uint16List
  ...
  void initState() {
    super.initState();
    tiles = List<HashMap<int, Tile>>.filled(maxZoom + 1, HashMap<int, Tile>());
    for (var i = 0; i <= maxZoom; i++) {
      tiles[i] = HashMap<int, Tile>();
    }
